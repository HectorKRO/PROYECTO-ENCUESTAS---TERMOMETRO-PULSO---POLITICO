-- ============================================================
-- V3.1: FIX — Re-seed de colonias con municipio_id
-- ============================================================
-- Versión: v3.1 (Hotfix)
-- Fecha: 2026-02-28
--
-- PROBLEMA:
--   La tabla `colonias` está vacía en producción (o tiene
--   municipio_id = NULL) porque el seed histórico
--   (seed_colonias_atlixco.sql) no incluía municipio_id,
--   y después de v3.0/01_catalogo_geografico.sql esa columna
--   quedó NOT NULL — haciendo imposible re-correr el seed.
--
-- SÍNTOMAS:
--   - "Selecciona una colonia (0 disponibles)" en FormularioEncuesta
--   - CACHE_VERSION bump invalidó el cache del browser,
--     exponiendo que Supabase tiene 0 colonias activas.
--
-- SOLUCIÓN:
--   1. Diagnóstico (mostrar estado actual)
--   2. Hacer municipio_id nullable temporalmente
--   3. Insertar las 417 colonias de Atlixco con ON CONFLICT DO NOTHING
--   4. Actualizar municipio_id vía JOIN con secciones_electorales
...
-- IDEMPOTENTE: Sí — ON CONFLICT DO NOTHING, safe para re-ejecutar
-- ============================================================

-- ============================================================
-- PASO 0: DIAGNÓSTICO (ver estado antes del fix)
-- ============================================================
DO $$
DECLARE
  v_total    INT;
...
BEGIN
  ...
  RAISE NOTICE '📊 DIAGNÓSTICO COLONIAS:';
  ...
  IF v_activas >= 417 THEN
    RAISE NOTICE '✅ Colonias ya están correctamente sembradas. No se requiere acción.';
  ELSE
    RAISE NOTICE '⚠️  Se procederá a sembrar las colonias faltantes...';
  END IF;
END $$;
...
INSERT INTO colonias (nombre, seccion_id, tipo, codigo_postal) VALUES
  ...
  ('LAS AMÉRICAS', '0164', 'COLONIA', '74200'),
  ...
  ('LAS MAÑANITAS', '0169', 'COLONIA', '74200'),
  ...
  ('RINCÓN DEL CIELO', '0174', 'COLONIA', '74470'),
  ...
  ('NUEVA ESPAÑA', '0175', 'COLONIA', '74200'),
  ...
  ('PARAÍSO ATLIXCO', '0181', 'FRACCIONAMIENTO', '74260'),
  ...
  ('FRACCIONAMIENTO PARAÍSO', '0182', 'FRACCIONAMIENTO', '74260'),
  ...
  ('RANCHO LA CAÑADA', '0184', 'RANCHO', '74360'),
  ...
  ('ACATLÁN DE OSORIO', '0186', 'COLONIA', '74200'),
  ...
  ('EL PEÑON', '0208', 'COLONIA', '74360'),
  ('LA JOYA', '0208', 'COLONIA', '74360'),
  ('RANCHO EL PEÑON', '0208', 'RANCHO', '74360'),
...
-- ============================================================
-- PASO 3: ASIGNAR municipio_id VÍA JOIN CON SECCIONES
-- ============================================================
UPDATE colonias co
...
-- ============================================================
-- PASO 4: FALLBACK — Atlixco (id=1) para cualquier huérfano
...
BEGIN
  ...
  IF v_sin_mun > 0 THEN
    RAISE EXCEPTION '❌ FALLO: Quedaron % colonias sin municipio_id. Revisar secciones_electorales.', v_sin_mun;
  END IF;

  IF v_activas < 400 THEN
    RAISE WARNING '⚠️  Solo % colonias activas en Atlixco (esperadas ≥400). Verificar datos.', v_activas;
  ELSE
    RAISE NOTICE '✅ Todo correcto. El formulario de encuesta debería mostrar las colonias.';
  END IF;
END $$;
